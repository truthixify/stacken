...
const MissionSubmissions: NextPage = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      if (filter !== 'ALL' && mission && stxAddress === mission.creatorAddress) {
        params.append('status', filter);
      }

      const response = await fetch(`/api/missions/${id}/submissions?${params}`);
      if (response.ok) {
        const data = await response.json();
  ...
  return (
    <Layout title={`Submissions - ${mission?.title || 'Mission'}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <button
          ...
          <h1 className="text-3xl font-bold text-gray-200 mb-2">Mission Submissions</h1>
          {mission && <h2 className="text-xl text-gray-200">{mission.title}</h2>}
        </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-gray-200 text-sm">
                  You can only view your own submissions. The mission creator will review all
            ...
            submissions.map(submission => (
              ...
              >
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-600/20">
                  ...
                  {/* Creator Actions */}
                  {isSignedIn &&
                    stxAddress === mission?.creatorAddress &&
