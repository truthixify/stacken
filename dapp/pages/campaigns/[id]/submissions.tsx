...
const CampaignSubmissions: NextPage = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'UNDER_REVIEW': return <Eye className="text-blue-500" size={20} />;
  ...
  const getSubmissionTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'SOCIAL_PROOF': return <Share2 size={16} />;
  ...
  const renderSubmissionContent = (submission: Submission) => {
    ...
    switch (submissionType) {
      ...
      case 'SOCIAL_PROOF':
        return (
          <div>
            <span className="text-gray-700">{content.socialHandle}</span>
          </div>
        );
      default:
        return null;
    ...
    <Layout title={`Submissions - ${campaign?.title || 'Campaign'}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Campaign Submissions</h1>
          {campaign && <h2 className="text-xl text-gray-600">{campaign.title}</h2>}
        </div>
        ...
        {isSignedIn && stxAddress !== campaign?.creatorAddress && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex items-start">
              ...
              <div>
                <h3 className="font-semibold text-blue-900 mb-1">Your Submissions</h3>
                <p className="text-blue-700 text-sm">
                  You can only view your own submissions. The campaign creator will review all submissions privately.
                </p>
              </div>
            </div>
          </div>
            ...
            submissions.map((submission) => (
              <div key={submission._id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900">
                        {submission.user?.displayName || submission.user?.username || formatAddress(submission.userAddress)}
                      </h3>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className={`text-sm font-medium px-2 py-1 rounded-full ${getStatusColor(submission.status)}`}>
                ...
                <div className="mb-4">
                  ...
                  {submission.content.description && (
                    <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-700">{submission.content.description}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                  <div className="flex items-center space-x-4">
                    {submission.pointsAwarded > 0 && (
                      <div className="flex items-center text-green-600">
                        ...
                        <span className="font-medium">{submission.pointsAwarded} points awarded</span>
                      </div>
                    )}
                  ...
                  {/* Creator Actions */}
                  {isSignedIn && stxAddress === campaign?.creatorAddress && submission.status === 'PENDING' && (
                    <div className="flex space-x-2">
