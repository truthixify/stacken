    ...
    try {
      ...
      if (response.ok) {
        ...
        setStats({
          ...
          totalPoints: data.stats?.totalPointsDistributed || data.users.reduce((sum: number, user: LeaderboardUser) => sum + user.totalPoints, 0),
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Be the first to participate in campaigns and earn points!</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-center space-x-4">
                      ...
                      {user.avatar ? (
                        ...
                        <div className="w-16 h-16 rounded-full bg-primary-100 flex items-center justify-center border-4 border-white shadow-lg">
                          <span className="text-primary-600 font-bold text-lg">
                            {(user.displayName || user.username || user.stacksAddress).charAt(0).toUpperCase()}
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="text-right">
                      <div className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-bold ${getRankBadge(user.rank)}`}>
                        {user.totalPoints.toLocaleString()} pts
                      </div>
                      <p className="text-xs text-gray-500 mt-1">Rank #{user.rank}</p>
                    </div>
                  </div>
                </div>
              ))}
